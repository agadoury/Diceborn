  ...
  return (
    <div className="safe-pad min-h-svh bg-arena-0 text-ink relative flex flex-col
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center gap-2 my-2
        ...
        >
          {(() => {
            // During the defensive flow show the defender's dice tumbling on
            // their roll, otherwise the active (attacking) player's tray.
            const defenseInFlight = !!(state.pendingAttack || state.pendingDefenseRoll);
            const defenderId = state.pendingAttack?.defender ?? state.pendingDefenseRoll?.defender;
...
function useDiceRollKey(viewer: PlayerId): number {
  ...
  // Bump on every `dice-rolled` or `defense-dice-rolled` event so the tray
  // tumbles for both offensive and defensive rolls.
